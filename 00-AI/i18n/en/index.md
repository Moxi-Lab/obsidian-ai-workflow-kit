---
type: vault-index
status: active
language: en
---

# Knowledge Base Home

> Humans read the structure here. AI agents start from `00-AI/START-HERE.md`.

## Core Entrypoints

- `00-AI/START-HERE.md`: the single AI startup entrypoint.
- `00-AI/AGENTS.md`: general agent rules.
- `00-AI/governance/README.md`: governance rules.
- `00-AI/pipeline/README.md`: local material intake workflow.
- `00-AI/recall/README.md`: recall system.
- `10-Projects/PROJECTS-REGISTRY.md`: project registry.

## Common Workflows

- Project handoff: `10-Projects/01-example-project/README.md`
- AI governance: `00-AI/governance/README.md`
- Local material intake: `00-AI/pipeline/local-material-intake.md`
- Source processing: `40-ExternalSources/README.md`
- Recall rules: `00-AI/recall/task-to-context-map.md`
- Lesson capture: `20-SharedAssets/02-modules/project-lesson-promotion-v1.md`
- Vault health check: `20-SharedAssets/02-modules/vault-health-checklist-v1.md`

## Template Entrypoints

- `00-AI/templates/TPL-project-bridge-card.md`
- `00-AI/templates/TPL-agent-handoff-card.md`
- `00-AI/templates/TPL-source-analysis-card.md`
- `00-AI/templates/TPL-task-state-card.md`
- `00-AI/templates/TPL-acceptance-record.md`
