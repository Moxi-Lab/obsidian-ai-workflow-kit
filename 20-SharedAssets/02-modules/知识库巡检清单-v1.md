...
- [ ] `02-MOCs/MOC-INDEX.md` 只列出真实存在的稳定入口
...
- [ ] 新增规则是否已接入 `AGENTS.md`、`00-Agent-OS-Lite/` 或 `20-SharedAssets/`
...
```text
按《知识库巡检清单 v1》执行一轮巡检：
1) 先检查 index / START-HERE / AGENTS / MOC-INDEX 是否可达；
2) 列出断链、旧链接、过时 MOC 和私有库概念残留；
3) 标出 Inbox 堆积点；
4) 标出重复、矛盾、过时、孤立内容；
5) 标出空项目、弱项目；
