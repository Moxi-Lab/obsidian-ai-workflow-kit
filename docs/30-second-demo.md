...
```text
You are the knowledge base maintenance agent. The root directory of this Obsidian vault is: <path-to-this-repository>. First read START-HERE.md, then use the read-only demo in examples/filled-example. Tell me the current project state, the latest decision, and the next action. Do not edit files.
```
